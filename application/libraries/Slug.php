...
class Slug
{
	...
	public $base_url = ''; //because we have an ssl version topside
	...
	public function create_uri($data = '', $id = '')
	{
		...
		if (is_array($data))
		{
			...
			{
				return $this->_check_uri($this->create_slug($this->create_slug($data[$this->title])), $id);
			}
		}
	...
	public function create_slug($string)
	{
		...
		// Librivox overwrite 
		$string = str_replace($this->base_url, '', $string);
	...
	private function _check_uri($uri, $id = FALSE, $count = 0)
	{
		...
		// Librivox replacement - we don't want to use -1, so no tag, then -2
